...
Usage:
    ...
    python cost_estimator.py --base                 # base build (100% vision target)
...
from config import (
    ...
    VISION_MODEL, MAX_WORKERS, EMBED_WORKERS, PAGE_DPI, VISION_TARGET, BASE_VISION_TARGET,
    MAX_EMBED_CHARS,
    EMBED_PRICE_PER_M_TOKENS_TOKENS, AVG_EMBED_TOKENS_VISION, AVG_EMBED_TOKENS_TEXT,
    AVG_VISION_TOKENS_INPUT, AVG_VISION_TOKENS_OUTPUT, AVG_VISION_SECONDS_PER_PAGE,
)
from ingest import (
...
SAMPLE_WORKERS = EMBED_WORKERS


def _sample_one(pdf: Path, page_num: int, model: str) -> tuple:
    doc = fitz.open(str(pdf))
    data_uri = render_page(doc[page_num - 1], dpi=PAGE_DPI)
    doc.close()
    t0 = time.time()
    _, usage = provider.transcribe_image(
        data_uri, "Transcribe this page.", model,
...
def sample_vision_tokens(targets: list, sample_n: int, model: str):
    ...
    print(
        f"\n  sampling {len(sample)} page(s) through {model} ({SAMPLE_WORKERS} workers)...")
...
def report(counts: dict, measured, vision_target: float, is_base: bool, model: str):
    ...
    price_in, price_out = provider._PRICING.get(model, (0.0, 0.0))
    ...
    if measured:
        ...
        avg_in, avg_out, avg_sec = AVG_VISION_TOKENS_INPUT, AVG_VISION_TOKENS_OUTPUT, AVG_VISION_SECONDS_PER_PAGE
        print(
            f"  estimated avg: {avg_in:,} in / {avg_out:,} out tokens  (run --sample N to measure)")

    tot_in = avg_in * n_vision
    tot_out = avg_out * n_vision
    cost_vis = tot_in / 1e6 * price_in + tot_out / 1e6 * price_out
    ...
    cost_embed = total_embed / 1e6 * EMBED_PRICE_PER_M_TOKENS

    print(
        f"\n  ── Embedding cost  (cohere-embed-v4 @ ${EMBED_PRICE_PER_M_TOKENS}/1M) ─────────")
...
def main():
    ...
    ap.add_argument("--model", default=None,
                    help=f"Override vision model for pricing/sampling (default: {VISION_MODEL})")
    args = ap.parse_args()
    ...
    measured = sample_vision_tokens(combined["targets"], args.sample, model)
    ...
    print(f"  {label}")
    print(f"{'=' * 60}")
    report(combined, measured, vision_target, is_base, model)
