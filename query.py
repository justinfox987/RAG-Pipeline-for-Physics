...
from config import (
    ...
    RETRIEVAL_MIN_SCORE, RETRIEVAL_FETCH_MULTIPLIER, RETRIEVAL_CONFIDENCE_THRESHOLD,
)
from ingest import TRANSCRIPTION_PROMPT
...
def retrieve_pages(topics: list[str], query: str, top_k: int,
    ...
    for topic in topics:
        ...
        k = min(top_k * RETRIEVAL_FETCH_MULTIPLIER, index.ntotal)
...
def stream_reason_generator(question, pages, model, images=None, extra_system=None):
    """Generator version of reason(): yields (chunk, None) per text chunk,
    then ('', usage_dict) as the final item.  Same content-building logic as reason()."""
    system = SYSTEM_PROMPT
    if extra_system:
        system = system + extra_system
    ...
    yield from provider.stream_reason(
        system_prompt=system,
        user_messages=[{"role": "user", "content": content}],
        model=model,
        temperature=0.2,
...
def make_hyde_doc(question: str) -> str:
    """Generate a hypothetical textbook passage that answers the question.
    Embedding this instead of the raw query closes the vocabulary gap between
    short natural-language questions and dense LaTeX-heavy index entries."""
    try:
        text, _ = provider.reason(
            system_prompt=(
                "You are a physics textbook. Write a concise 3-5 sentence passage, "
                "including relevant LaTeX math, that directly answers the question. "
                "Output only the passage — no preamble, no commentary."
            ),
            user_messages=[{"role": "user", "content": question}],
            model=ROUTING_MODEL,
            temperature=0.1,
...
def decompose_query(question: str) -> list[str]:
    """Break a complex question into up to 3 focused retrieval sub-queries.
    Returns [question] unchanged for simple single-concept questions, so the
    caller doesn't need to special-case the single-query path."""
    try:
        text, _ = provider.reason(
            system_prompt=(
                "You are a retrieval query planner for a physics research assistant. "
                "Break the question into 1-3 focused sub-queries for document retrieval. "
                "Each sub-query should target a distinct concept, formula, or derivation "
                "step needed to fully answer the question. "
                "For simple single-concept questions return exactly 1 sub-query. "
                "Output only a JSON array of strings, e.g. [\"sub-query 1\", \"sub-query 2\"]. "
                "No explanation."
            ),
            user_messages=[{"role": "user", "content": question}],
            model=ROUTING_MODEL,
...
def filter_relevant_pages(question: str, pages: list[dict]) -> list[dict]:
    """Drop pages that a cheap LLM judge deems irrelevant to the question.
    Runs all judgements in parallel; keeps ordering by score."""
    if not pages:
        return pages

    def judge_one(page: dict) -> tuple[dict, bool]:
        snippet = page.get("description", "")[:400]
        try:
            text, _ = provider.reason(
                system_prompt=(
                    "You are a relevance filter for a physics research assistant. "
                    "Reply with only YES or NO. "
                    "Does this page contain information that would help answer the question?"
                ),
                user_messages=[{"role": "user", "content": (
                    f"Question: {question}\n\n"
                    f"Page ({page['source']} p.{page['page_num']}):\n{snippet}"
                )}],
                model=ROUTING_MODEL,
                temperature=0.0,
                max_tokens=5,
            ...
            return page, text.strip().upper().startswith("Y")
        except Exception:
            return page, True  # keep on failure — don't silently drop

    with ThreadPoolExecutor(max_workers=len(pages)) as executor:
        results = list(executor.map(lambda p: judge_one(p), pages))

    kept = [page for page, relevant in results if relevant]
    print(f"  relevance filter: {len(kept)}/{len(pages)} pages kept")
    return kept


def expand_with_neighbors(pages: list[dict]) -> list[dict]:
    """For each retrieved page, append the immediately adjacent pages (N±1) from
    the same source if they aren't already in the list.  Neighbors are scored at
    90% of their parent's score so they sort below the directly retrieved pages."""
    if not pages:
        return pages

    topics_needed = sorted({p["topic"] for p in pages})
    meta_by_topic: dict[str, dict] = {}
    for topic in topics_needed:
        mp = INDEXES_DIR / topic / "metadata.json"
        if mp.exists():
            with open(mp, "r", encoding="utf-8", errors="replace") as f:
                meta_by_topic[topic] = json.load(f)

    # (source, page_num) → (faiss_idx, page_record) per topic
    lookup: dict[str, dict[tuple, tuple]] = {}
    for topic, meta in meta_by_topic.items():
        lookup[topic] = {}
        for idx, rec in enumerate(meta["pages"]):
            lookup[topic][(rec["source"], rec["page_num"])] = (idx, rec)

    included = {(p["topic"], p["source"], p["page_num"]) for p in pages}
    result = list(pages)
    ...
    for parent in pages:
        ...
        if topic not in lookup:
            continue
        for delta in (-1, +1):
            key = (parent["source"], parent["page_num"] + delta)
            if key not in lookup[topic]:
                continue
            idx, rec = lookup[topic][key]
            uid = (topic, rec["source"], rec["page_num"])
            if uid in included:
                continue
            included.add(uid)
            neighbor = rec.copy()
            neighbor["score"] = round(parent["score"] * 0.9, 4)
            neighbor["topic"] = topic
            neighbor["_idx"] = idx
...
def rrf_merge(ranked_lists: list[list[dict]], k: int = 60) -> list[dict]:
    """Merge multiple ranked page lists with Reciprocal Rank Fusion.
    'score' on each output page is the best cosine similarity seen across all
    lists, keeping downstream thresholds (lazy upgrade, display) meaningful."""
    rrf_scores: dict[tuple, float] = {}
    best_cosine: dict[tuple, float] = {}
    page_by_key: dict[tuple, dict] = {}

    for ranked in ranked_lists:
        for rank, page in enumerate(ranked):
            key = (page["topic"], page["_idx"])
            rrf_scores[key] = rrf_scores.get(key, 0.0) + 1.0 / (rank + k + 1)
            cosine = page["score"]
            if key not in best_cosine or cosine > best_cosine[key]:
                best_cosine[key] = cosine
                page_by_key[key] = page.copy()

    result = []
    for key in sorted(rrf_scores, key=lambda key: -rrf_scores[key]):
        page = page_by_key[key].copy()
        page["score"] = best_cosine[key]
        result.append(page)
    return result
...
def run_query(
    ...
    if force_retrieval or _should_retrieve(retrieval_query):
        ...
        for sub_q in sub_queries:
            sub_pages, sub_best = retrieve_pages(topics, sub_q, top_k, min_score=RETRIEVAL_MIN_SCORE)
            ranked_lists.append(sub_pages)
            if sub_best is not None:
                all_bests.append(sub_best)

        hyde_doc = make_hyde_doc(retrieval_query)
        hyde_pages, hyde_best = retrieve_pages(topics, hyde_doc, top_k, min_score=RETRIEVAL_MIN_SCORE)
        ranked_lists.append(hyde_pages)
        if hyde_best is not None:
            all_bests.append(hyde_best)

        best_score = max(all_bests) if all_bests else None

        if best_score is None or best_score < RETRIEVAL_CONFIDENCE_THRESHOLD:
            print(f"  low confidence (best={best_score:.3f if best_score else 'none'}) — skipping retrieval")
            pages = []
        else:
            pages = rrf_merge(ranked_lists)[:top_k]
            if not disable_upgrades:
                upgrade_raw_math_pages(pages)
            pages = filter_relevant_pages(question, pages)
            pages = expand_with_neighbors(pages)
    else:
        pages = []
